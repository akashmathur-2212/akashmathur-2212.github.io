...
- Engineered a `CI/CD` serverless workflow leveraging `AWS`, `Docker`, `Terraform` and `GitHub Actions` to streamline the model deployment and serve with `FastAPI`. Setup `monitoring` with SageMaker Model Monitor to track its performance and identify any issues.
